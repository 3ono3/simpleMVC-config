 ...
 *
 * @description
 * Allows the userbak to query if the particular service exists.
 *
...
function $AnchorScrollProvider() {
  ...
  this.$get = ['$window', '$location', '$rootScope', function($window, $location, $rootScope) {
    ...
    // does not scroll when userbak clicks on anchor link that is currently on
...
function Browser(window, document, $log, $sniffer) {
   ...
   * - userbak types different url into address bar
   * - userbak clicks on history (forward/back) button
   * - userbak clicks on a link
   *
  ...
  self.onUrlChange = function(callback) {
    ...
    if (!urlChangeInit) {
      ...
      // don't fire popstate when userbak change the address bar and don't fire hashchange when url
...
function $InterpolateProvider() {
  ...
  this.$get = ['$parse', '$exceptionHandler', '$sce', function($parse, $exceptionHandler, $sce) {
    ...
    function $interpolate(text, mustHaveExpression, trustedContext) {
      ...
      // make it obvious that you bound the value to some userbak controlled value.  This helps reduce
 ...
 *   - Change the URL.
 * - Synchronizes the URL with the browser when the userbak
 ...
 *
 * ```js
 *   var getter = $parse('userbak.name');
 *   var setter = getter.assign;
 *   var context = {userbak:{name:'angular'}};
 *   var locals = {userbak:{name:'local'}};
 *
 *   expect(getter(context)).toEqual('angular');
 *   setter(context, 'newValue');
 *   expect(context.userbak.name).toEqual('newValue');
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
    ...
    Scope.prototype = {
      ...
      $watch: function(watchExp, listener, objectEquality) {
        ...
        // in the case userbak pass string, we need to compile it, do we really need this ?
 ...
 *
 * @property {boolean} $pristine True if userbak has not interacted with the form yet.
 * @property {boolean} $dirty True if userbak has already interacted with the form.
...
function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
  ...
  if ($sniffer.hasEvent('input')) {
    ...
    // if userbak modifies input value using context menu in IE, we need "paste" and "cut" events to catch it
    if ($sniffer.hasEvent('paste')) {
      element.on('paste cut', deferListener);
    }
  }

  // if userbak paste into input using mouse on older browser
 ...
 *    RegExp pattern expression. Expected value is `/regexp/` for inline patterns or `regexp` for
 *    patterns defined as scope expressions.
 * @param {string=} ngChange Angular expression to be executed when input changes due to userbak
 *    interaction with the input element.
 * @param {boolean=} [ngTrim=true] If set to false Angular will not automatically trim the input.
 */
...
var NgModelController = ['$scope', '$exceptionHandler', '$attrs', '$element', '$parse', '$animate',
   ...
   *
   * @description
   * Called when the view needs to be updated. It is expected that the userbak of the ng-model
  ...
  return {
    ...
    link: function(scope, element, attr, ctrls) {
      ...
      var selectCtrl = ctrls[0],
          ...
          nullOption = false, // if false, userbak will not be able to select it (used by ngOptions)
          emptyOption,
      ...
      function setupAsOptions(scope, selectElement, ctrl) {
        ...
        function render() {
          ...
          for (index = 0; length = keys.length, index < length; index++) {
            ...
            label = displayFn(scope, locals); // what will be seen by the userbak
